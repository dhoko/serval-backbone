...
(function(win, doc, App){
  ...
  App.Views.MasterView = Backbone.View.extend({
    ...
    initialize: function initialize() {
      ...
      this.events = _.extend({},this.events,App.Views.MasterView.prototype.events);
    },
    ...
    render: function render() {
      ...
      var context = _.extend({},App.Languages.Instances[App.Languages.current] || {},this.beforeRender());

      console.debug("[AbstractView@render] : Bind context to the view - " + Backbone.history.fragment + " at #" + this.el.id,context);
    ...
    toggleClassName : function toggleClassName() {
      ...
      var current = doc.body.className.match(/page-.\w*-?\w*/g);
      if(current) {
          current.map(function (item) {
              doc.body.classList.remove(item);
          });
      }

      this.bodyClass && doc.body.classList.add(this.bodyClass);
    },
